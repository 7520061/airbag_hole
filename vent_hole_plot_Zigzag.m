function vent_hole_plot_Zigzag(input, x_opt)
% ベントホール3個以下未対応
% close all
global DIRECTORY
...
bag_H_round = 0.25 * 0.5 * 1000;    % エアバッグ円弧部高さ(片側)
...
for i = 1:1:n
    ...
    if(rem(i, 2) == 0)
        x = vent_r(i) * cosd(a) + x_req;
    else
        x = vent_r(i) * cosd(a);
    end
    y = vent_r(i) * sind(a) + vent_H_sorted(i);
    plot(x,y,'linewidth',lWidth2,'color',color(i))
    ...
    if i <= 2
        ymax = vent_H_sorted(1)+20;
        if i== 1            
            txt = text('String','x=0',...
            'FontSize',11, 'FontName',fName,'Position',[-25 ymax+15 0],'Color','k');
        else
            txt = text('String',strcat('x=',num2str(x_req)),...
            'FontSize',11, 'FontName',fName,'Position',[-25+x_req ymax+15 0],'Color','k');
        end
        ymin = -vent_H_sorted(1)-50;
        plot([0 0],[ymax ymin],'-.','linewidth',0.5,'color','k');
        plot([x_req x_req],[ymax ymin],'-.','linewidth',0.5,'color','k');      
    end

    if rem(i, 2) == 0
        txt = text('String',strcat('D=',num2str(vent_D_sorted(i))),...
            'FontSize',12, 'FontName',fName,'Position',[30+x_req vent_H_sorted(i) 0],'Color',color(i));
    elseif rem(i, 2) == 1
        txt = text('String',strcat('D=',num2str(vent_D_sorted(i))),...
            'FontSize',12, 'FontName',fName,'Position',[30+x_req -vent_H_sorted(i) 0],'Color',color(i));
    end
...
% saveas(f(1),[DIRECTORY '/vent_hole.fig']);
% saveas(f(1),[DIRECTORY '/vent_hole.png']);
